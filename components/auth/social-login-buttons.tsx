...
import { signInWithOAuth, type AuthProvider } from "@/lib/auth";
...
const PROVIDERS: {
  ...
  { id: "google", label: "Continuar con Google", icon: <GoogleIcon />, enabled: true },
  { id: "github", label: "Continuar con GitHub", icon: <GitHubIcon />, enabled: true },
  { id: "discord", label: "Continuar con Discord", icon: <DiscordIcon />, enabled: true },
];
...
export function SocialLoginButtons({
  ...
  return (
    <div className={`space-y-2 ${className}`}>
      {PROVIDERS.filter((p) => p.enabled).map(({ id, label, icon }) => (
        <Button
          key={id}
          type="button"
          variant="outline"
